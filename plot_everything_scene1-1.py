...
def box_plot(data):
	...
	data = pd.concat([opt_d100, opt_d75, rl_d100, rl_d75, rf_d100, rf_d75, dt_d100, dt_d75, std128_d100, std128_d75]) # opt_d100, opt_d75, rl_d100, rl_d75, rf_d100, rf_d75, dt_d100, dt_d75, std16_d100, std16_d75, std32_d100, std32_d75, std64_d100, std64_d75, std128_d100, std128_d75, std256_d100, std256_d75 
	...
	for i in [0,2,4,6,8]:# 0,2,4,6,8,10,12,14,16
		mybox = ax.artists[i]
		#mybox.set_facecolor('red')
		mybox.set_edgecolor('red')
		mybox.set_linewidth(1.0)
	for i in [1,3,5,7,9]:# 1,3,5,7,9,11,13,15,17
		mybox = ax.artists[i]
...
def get_all_data(episode_num,step_num,ps,file_type,txt_num,std_init=''):
	...
	if std_init == '':
		for txt_number in range(1,txt_num+1):
			...
			data_path = './Dataset/plotDatas/Markovian/'+file_type+'/'+file_name+'/'+file_name+'_'+str(txt_number)+'.txt'
		...
		for txt_number in range(1,txt_num+1):
			file_name = str(episode_num)+'_'+str(step_num)+'_'+str(int(ps*100))
			data_path = './Dataset/plotDatas/Markovian/'+file_type+'/'+file_name+'/'+str(std_init)+'/'+file_name+'_'+str(txt_number)+'.txt'
			temp_data = load_data(data_path)
			all_data.append(temp_data)
		return np.array(all_data)
