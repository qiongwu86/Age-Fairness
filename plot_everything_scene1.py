...
def box_plot(data):
	...
	data = pd.concat([opt_d100, opt_d75, rl_d100, rl_d75, rf_d100, rf_d75, dt_d100, dt_d75, std64_d100, std64_d75, std128_d100, std128_d75]) # opt_d100, opt_d75, rl_d100, rl_d75, rf_d100, rf_d75, dt_d100, dt_d75, std16_d100, std16_d75, std32_d100, std32_d75, std64_d100, std64_d75, std128_d100, std128_d75, std256_d100, std256_d75 
	fig, ax = plt.subplots()
	...
	for i in [0,2,4,6,8,10]:# 0,2,4,6,8,10,12,14,16
		mybox = ax.artists[i]
		#mybox.set_facecolor('red')
		mybox.set_edgecolor('red')
		mybox.set_linewidth(1.0)
	for i in [1,3,5,7,9,11]:# 1,3,5,7,9,11,13,15,17
		mybox = ax.artists[i]
	...
	ax.set_ylim([0.84,1]) # [0.725,1]
	# ax.set_title('Comparison of approach fairness'+'\n'+'(Max device = 6, arrival rate = departure rate = 3)')
	ax.grid(linestyle=':')
	plt.show()
...
if __name__ == '__main__':
	# episode_num_1,step_num_1,ps_1,file_type_r,txt_num = 200,200,1.0,'all_rewards',10 # 'all_rewards''test_data'
	# episode_num_2,step_num_2,ps_2,file_type_r,txt_num,std_init = 200,400,0.75,'all_rewards',10,35 #
	# #episode_num_3,step_num_3,ps_3,file_type_r,txt_num,std_init = 200,400,0.75,'all_rewards',10,35 #
	# all_rewards_1 = np.sum(get_all_data(episode_num_1,step_num_1,ps_1,file_type_r,txt_num),axis=0)
	# all_rewards_2 = np.sum(get_all_data(episode_num_2,step_num_2,ps_2,file_type_r,txt_num,std_init),axis=0)
	# #all_rewards_3 = np.sum(get_all_data(episode_num_3,step_num_3,ps_3,file_type_r,txt_num,std_init),axis=0)
	# avg_rewards_1 = all_rewards_1/txt_num
	# avg_rewards_2 = all_rewards_2/txt_num
	# #avg_rewards_3 = 
	# #all_rewards_3/txt_num

	# x_len = len(avg_rewards_1)
	# x_range = [x for x in range(1,x_len+1)]

	# plt.plot(x_range, [1]*x_len, color='red', label='Absolute fair limit')
	# plt.plot(x_range, avg_rewards_1, label='Rainbow traning eposide average rewards, ps=1.0')
	# plt.plot(x_range, avg_rewards_2, label='Rainbow traning eposide average rewards, ps=0.75')
	# # plt.plot(x_range, avg_rewards_3, label='Rainbow traning average rewards, ps=0.5')
	# plt.grid(linestyle=':')
	# plt.legend()
	# plt.xlabel('Episodes')
	# plt.ylabel('Age fairness utility')
	# plt.title('Rainbow traning rewards')
	# plt.show()
	...
	txtList = ['OPT','RL','RF','DT','SP16','SP32','SP64','SP128','SP256'] # 'OPT','RL','RF','DT','SP16','SP32','SP64','SP128','SP256'
